...
encodeBigIntTests =
    describe "Cbor.Encode.bigInt"
        ...
        , test "negative max 64-bit" <|
            \_ -> encodeToHex (CE.bigInt Negative (Hex.toBytesUnchecked "ffffffffffffffff")) |> Expect.equal "3bffffffffffffffff"

        -- Width 9+: bignum (tag 2/3 + byte string)
        , test "positive 9-byte → tag 2 bignum" <|
            \_ -> encodeToHex (CE.bigInt Positive (Hex.toBytesUnchecked "010000000000000000")) |> Expect.equal "c249010000000000000000"
        , test "negative 9-byte → tag 3 bignum" <|
            \_ -> encodeToHex (CE.bigInt Negative (Hex.toBytesUnchecked "010000000000000000")) |> Expect.equal "c349010000000000000000"
        , test "positive 16-byte bignum" <|
        ...
        , test "round-trip: 8-byte magnitude" <|
            \_ ->
                CE.encode (CE.bigInt Negative (Hex.toBytesUnchecked "0100000000000000"))
                    |> CD.decode CD.bigInt
                    |> Result.map (\( s, bs ) -> ( s, Hex.fromBytes bs ))
                    |> Expect.equal (Ok ( Negative, "0100000000000000" ))
        , test "round-trip: 9-byte bignum" <|
            \_ ->
                CE.encode (CE.bigInt Positive (Hex.toBytesUnchecked "010000000000000000"))
                    |> CD.decode CD.bigInt
                    |> Result.map (\( s, bs ) -> ( s, Hex.fromBytes bs ))
                    |> Expect.equal (Ok ( Positive, "010000000000000000" ))
        ]
